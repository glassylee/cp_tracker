      ...
      <div className="space-y-5">
        {materials.map((m: any) => (
          <div key={m.id} className="flex items-center gap-4">
            ...
            <input type="text" inputMode="numeric" value={qty[m.id] || ""} onChange={e => setQty(p => ({ ...p, [m.id]: e.target.value }))} style={inputStyle} className="h-14 w-28 border-2 border-slate-300 rounded-2xl px-4 text-center font-black text-lg" placeholder="0" />
    ...
    <form onSubmit={handleSubmit} className="p-8 bg-white rounded-3xl border border-slate-200 shadow-sm space-y-8">
      ...
      <div className="space-y-6">
        <div>
          ...
          <input type="text" inputMode="decimal" value={temp} onChange={e => setTemp(e.target.value)} style={inputStyle} className="h-14 w-full border-2 border-slate-300 rounded-2xl px-6 font-black text-xl" placeholder="예: 25.5" />
        </div>
